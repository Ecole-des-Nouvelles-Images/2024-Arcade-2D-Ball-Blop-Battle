<div align="center">
  <a href="https://github.com/Ecole-des-Nouvelles-Images/2024-Arcade-2D-Ball-Blop-Battle/blob/main/README.md">
    <img width="64" height="64" alt="flag_angleterre" src="https://github.com/user-attachments/assets/79223af9-d6d8-48a1-aa17-7355c3c6b444" />
  </a>
  &nbsp;&nbsp;&nbsp;
  <a href="https://github.com/Ecole-des-Nouvelles-Images/2024-Arcade-2D-Ball-Blop-Battle/blob/main/README.es.md">
    <img width="64" height="64" alt="flag_espagne" src="https://github.com/user-attachments/assets/22639270-f31b-4416-9445-73230a53ec00" />
  </a>
  &nbsp;&nbsp;&nbsp;
  <a href="https://github.com/Ecole-des-Nouvelles-Images/2024-Arcade-2D-Ball-Blop-Battle/blob/main/README.fr.md">
    <img width="64" height="64" alt="flag_france" src="https://github.com/user-attachments/assets/b98ca283-5441-4f57-a475-8cbf1f843485" />
  </a>
</div>

<div align="center">
  <h1>Ball Blop Battle</h1>
</div>

![IllustrationLoadingScreen](https://github.com/user-attachments/assets/a2b3d9a9-95d4-4461-aa9f-42438d908716)
[![Unity Version](https://img.shields.io/badge/Unity-2022.3.47-blue?style=flat&logo=unity)](https://unity.com/)
[![GitHub Release](https://img.shields.io/github/release/Ecole-des-Nouvelles-Images/2024-Arcade-2D-Ball-Blop-Battle)](https://github.com/Ecole-des-Nouvelles-Images/2024-Arcade-2D-Ball-Blop-Battle/releases)
[![License](https://img.shields.io/github/license/Ecole-des-Nouvelles-Images/2024-Arcade-2D-Ball-Blop-Battle)](https://github.com/Ecole-des-Nouvelles-Images/2024-Arcade-2D-Ball-Blop-Battle/blob/main/LICENSE)
[![Repo Size](https://img.shields.io/github/repo-size/Ecole-des-Nouvelles-Images/2024-Arcade-2D-Ball-Blop-Battle?color=lightgrey)](https://github.com/Ecole-des-Nouvelles-Images/2024-Arcade-2D-Ball-Blop-Battle)

**Ball Blop Battle** es un juego **arcade de deportes y acción** en vista lateral 1vs1. El jugador controla un **blop** para **luchar** contra sus oponentes en un partido de **vóley** revisitado.

## Tabla de Contenidos
1. [Introducción](#introducción)
2. [Universo](#universo)
3. [Equipo](#equipo)
4. [Jugabilidad](#jugabilidad)
6. [Lanzamiento](#lanzamiento)
7. [Desarrollo](#desarrollo)
8. [Agradecimientos Especiales](#agradecimientos-especiales)
9. [Recomendaciones](#recomendaciones)

---
<div align="center">
  <h1>Introducción</h1>
</div>

**Ball Blop Battle** es un juego de **arcade, deportes y acción** desarrollado durante **cinco semanas**.  
Este proyecto fue creado como parte de **un proyecto escolar**.  
El proceso de desarrollo incluyó **una preproducción (1 semana), producción (3 semanas) y postproducción (1 semana)**.

---
<div align="center">
  <h1>Universo</h1>
</div>

Los **blops**, pequeñas criaturas redondas hechas de baba con **habilidades especiales**, participan en la Batalla de Blops a través de diferentes **arenas**.

---
<div align="center">
  <h1>Equipo</h1>
</div>

Este juego fue creado por:

**[Marion DUBREIL](https://github.com/Zamelkhia)** - *Artista de Juego*  
**[Calvin OVIDE](https://github.com/CalvinOvide)** - *Artista de Juego*  
**[Hugo ISAMBART](https://github.com/HIsambart)** - *Desarrollador de Juego*

![baniere_readme3](https://github.com/user-attachments/assets/163b3ef1-d7d9-4fd6-9f82-01021287457a)

---
<div align="center">
  <h1>Jugabilidad</h1>
</div>

**Ball Blop Battle** es un juego **1vs1** en el que cada jugador controla un blop y debe **ganar** un partido de vóley revisitado.

## Blops
Hay **4 blops diferentes**, cada uno con su propio **ataque especial** y diferenciados principalmente por su **color**:

<div align="center">
  <img src="https://github.com/user-attachments/assets/d6156c0e-b9c4-464a-82c8-40555a30213f" alt="Ejemplo de imagen" style="max-width: 100%; height: auto;">
</div>

## Mecánicas del juego
Cada jugador puede **moverse**, **saltar**, **deslizarse (dash)**, **atacar**, **atrapar** y **recibir** la pelota.  
Los jugadores solo pueden **moverse** cuando **no tienen la pelota**.

https://github.com/user-attachments/assets/21fd09d4-ce86-439a-9fcd-63709f87ada4

---
<div align="center">
  <h1>Lanzamiento</h1>
</div>

El juego está disponible en Windows, y aquí está el enlace para **[descargarlo](https://github.com/Ecole-des-Nouvelles-Images/2024-Arcade-2D-Ball-Blop-Battle/releases)**.

Este juego es compatible con **Windows**.

---
<div align="center">
  <h1>Desarrollo</h1>
</div>

Este proyecto está actualmente **en desarrollo**. Las futuras actualizaciones podrían incluir **nuevas arenas, nuevos personajes y correcciones de errores**.  

Si encuentras algún error o tienes sugerencias, por favor abre un **[ticket de incidencia](https://github.com/Ecole-des-Nouvelles-Images/2024-Arcade-2D-Ball-Blop-Battle/issues/new)**.

---
<div align="center">
  <h1>Agradecimientos Especiales</h1>
</div>

Nos gustaría agradecer a nuestra **escuela** y a nuestros **profesores**:  
- **[L'École des Nouvelles Images](https://www.nouvellesimages.xyz/)**
- **[Frédéric BAST](https://github.com/FBast)** - *Soporte de Desarrollo*
- **[Yvan BLADET](https://www.linkedin.com/in/yvan-bladet-676924261/)** - *Soporte de Desarrollo / Integración*
- **[Jerome CROS](https://fr.linkedin.com/in/crospro)** - *Soporte Artístico*
- **[Tommy HA PHUOC](https://fr.linkedin.com/in/tommyhaphuoc/en)** - *Soporte de Gameplay*

---
<div align="center">
  <h1>Recomendaciones</h1>
</div>

Para una mejor experiencia, recomendamos jugar con **mandos (controladores)**.
