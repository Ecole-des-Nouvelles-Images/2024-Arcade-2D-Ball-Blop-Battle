...
namespace _Branches.Hugo.OldScripts.Player
{
    public class OldPlayerController : MonoBehaviour
    {
        ...
        private void Update()
        {
            ...
            {
                _oldPlayerNumberTouchBallHandler.Fouls();
                _hasTheBall = false;
            }
            
            // Reset States End of Timer
            if (OldMatchManager.IsSetOver)
            {
                _hasTheBall = false;
        ...
        private void OnCollisionEnter2D(Collision2D other)
        {
            if (other.gameObject.CompareTag("Ball"))
            {
                ...
                {
                    _ball.GetComponent<OldBallHandler>().PerfectReception();
                ...
                if (_isDashing)
                {
                    _ball.GetComponent<OldBallHandler>().DashReception();
                }
                ...
                if (Mathf.Approximately(_isWestButtonPressed, 1) && !_isGrounded && _oldPlayerNumberTouchBallHandler.NumberTouchBall < 2)
                {
                    // && _playerNumberTouchBallHandler.NumberTouchBall < 2
                    
                    _ball.GetComponent<OldBallHandler>().IsAbsorb(gameObject);
                    _hasTheBall = true;
                    _isDashing = false;
                    _canMove = true;
                ...
                if(_isWestButtonPressed == 0 && _oldPlayerNumberTouchBallHandler.NumberTouchBall < 2 && !_hasPunch)
                {
                    _ball.GetComponent<OldBallHandler>().IsPunch(direction, _rb2d.velocity);
                ...
                if (IsSpecialSpike && _oldPlayerNumberTouchBallHandler.NumberTouchBall < 2 && !_isGrounded)
                {
                    _hasTheBall = true;
                    CanAbsorb = false;
                    ...
                    if (_oldPlayerNumberTouchBallHandler.IsPlayerOne)
                    {
                ...
                else if (IsSpecialSpike && _oldPlayerNumberTouchBallHandler.NumberTouchBall < 2 && _isGrounded)
                {
                    IsSpecialSpike = false;
                    CanSpecialSpike = true;
                }

                if (IsSpecialSpike && _oldPlayerNumberTouchBallHandler.NumberTouchBall == 3)
                {
                    IsSpecialSpike = false;
                    CanSpecialSpike = true;
                }
        ...
        public void GetWestButtonReadValue(float buttonValue)
        {
            ...
            if (Mathf.Approximately(buttonValue, 1) && _move == Vector2.zero && _isGrounded && _oldPlayerNumberTouchBallHandler.NumberTouchBall < 1)
            {
                _canPerfectReception = true;
                Invoke(nameof(ReverseCanPerfectReception), _timeCanPerfectReception);
            }

            if (Mathf.Approximately(buttonValue, 1) && !_isGrounded && _oldPlayerNumberTouchBallHandler.NumberTouchBall < 2)
            {
                CanAbsorb = true;
            ...
            {
                _ball.GetComponent<OldBallHandler>().IsDrawn(_move);
                _hasTheBall = false;
        ...
        public void GetEastButtonReadValue(float buttonValue)
        {
            ...
            {
                if (CanSpecialSpike && _oldPlayerNumberTouchBallHandler.IsPlayerOne == _ball.GetComponent<OldBallHandler>().IsPlayerOneSide)
                {
                    _ball.GetComponent<OldBallHandler>().SpecialSpikeActivation();
                ...
                if (_blop.PlayerName == "Vert" && CountShootSpecialSpike == 1)
                {
                    ...
                    // VFX
                    if (_ball.GetComponent<OldBallHandler>().VFXSpecialSpikeGreen.activeSelf)
                    {
                        Debug.Log("Special Spike Green Play");
                        _ball.GetComponent<OldBallHandler>().VFXSpecialSpikeGreenImpact.Play();
                    }
                    return;
                }
            }
